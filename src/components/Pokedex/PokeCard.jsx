...
const PokeCard = ({ pokemon }) => {
  ...
  return (
    <article onClick={handleClik} className={`card border-${poke?.types[0].type.name}`}>
      <header className={`card_header bg-${poke?.types[0].type.name}`}>
        <img  className='card_avatar' 
      ...
      <ul className='card_stat-list'>
        {
          poke?.stats.map(stat => (
            <li className='card_stat-item' key={stat.stat.url}> 
              <span className='card_stat_name'>{stat.stat.name}</span>
              <span className={`card_stat_name card_stat-number color-${poke?.types[0].type.name}`}>{stat.base_stat}</span>
            </li>
          ))
        }
      </ul>
